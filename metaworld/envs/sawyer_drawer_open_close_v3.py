...
class SawyerDrawerOpenCloseEnvV3(SawyerXYZEnv):
    def __init__(
        self,
    ...
    def evaluate_state(
        ...
        if self.init_pos is None:
            self.init_pos = obs[4:7]
        self.has_been_opened = max(float(handle_error <= 0.03), self.has_been_opened)
        self.has_been_closed = self.has_been_opened * max(float(np.linalg.norm(obs[4:7] - self.init_pos) <= 0.055), self.has_been_closed)
        
        info = {
            "success": self.has_been_closed * self.has_been_opened,
